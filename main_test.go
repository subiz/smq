...
func TestQueue(t *testing.T) {
	...
	if !found || "" != jobid || state != "" || len(jobs) != 2 ||
		jobs[0].JobID != j1 || jobs[0].Value != "1" ||
		jobs[1].JobID != j2 || jobs[1].Value != "2" {
	...
	if len(jobs) != 3 ||
		jobs[0].JobID != j1 || jobs[0].Value != "1" ||
		jobs[1].JobID != j2 || jobs[1].Value != "2" ||
		jobs[2].JobID != j3 || jobs[2].Value != "3" {
		t.Errorf("wrong")
	}
	queue.Commit(par, q, j2, "good")
	found, jobid, state, jobs = queue.Peek(par, q, 2)
	if !found || jobid != j2 || state != "good" || len(jobs) != 1 ||
		jobs[0].JobID != j3 || jobs[0].Value != "3" {
